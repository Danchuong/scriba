"""Unit tests for :func:`scriba.animation.errors._suggest_closest`.
...
class TestSuggestClosest:
    def test_close_typo_returns_suggestion(self) -> None:
        ...
        assert _suggest_closest("currnet", ["current", "done", "idle"]) == "current"

    def test_dropped_char_returns_suggestion(self) -> None:
        assert _suggest_closest("idl", ["current", "done", "idle", "dim"]) == "idle"

    def test_far_off_returns_none(self) -> None:
        # "zzzzzzz" is nothing like any candidate — should return None.
        assert _suggest_closest("zzzzzzz", ["current", "done", "idle"]) is None

    def test_empty_candidates_returns_none(self) -> None:
        assert _suggest_closest("anything", []) is None

    def test_exact_match_returns_self(self) -> None:
        assert _suggest_closest("done", ["current", "done", "idle"]) == "done"

    def test_picks_best_match(self) -> None:
        # When multiple candidates are close, difflib should pick the best.
        result = _suggest_closest("xranges", ["xrange", "yrange"])
    ...
    def test_custom_cutoff_accepts_weaker_match(self) -> None:
        ...
        weak = _suggest_closest("actve", ["idle", "current"], cutoff=0.3)
    ...
    def test_iterable_input_not_just_list(self) -> None:
        ...
        assert _suggest_closest("currnet", ("current", "done")) == "current"
