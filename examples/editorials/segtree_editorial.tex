...
\recolor{T.node["[0,1]"]}{state=current}
\narrate{Di xuong nhanh trai cua \texttt{[0,2]}: node \texttt{[0,1]}. Doan $[0,1]$ khong nam hoan toan trong $[1,4]$ (vi co phan tu $0$), nen tiep tuc chia.}

\step
\recolor{T.node["[0,1]"]}{state=dim}
\recolor{T.node["[0,0]"]}{state=current}
\narrate{Nhanh trai \texttt{[0,0]}. Doan $[0,0]$ nam ngoai $[1,4]$ -- bo qua, tra ve $0$.}

\step
\recolor{T.node["[0,0]"]}{state=dim}
\recolor{T.node["[1,1]"]}{state=current}
\narrate{Nhanh phai \texttt{[1,1]}. Doan $[1,1]$ nam hoan toan trong $[1,4]$ -- lay ngay gia tri $a[1] = 3$.}

\step
\recolor{T.node["[1,1]"]}{state=good}
\recolor{T.edge[("[0,1]","[1,1]")]}{state=good}
\recolor{T.node["[2,2]"]}{state=current}
\narrate{Quay len \texttt{[0,2]}, di sang nhanh phai \texttt{[2,2]}. Doan $[2,2]$ nam hoan toan trong $[1,4]$ -- lay $a[2] = 5$.}

\step
\recolor{T.node["[2,2]"]}{state=good}
\recolor{T.edge[("[0,2]","[2,2]")]}{state=good}
\narrate{Hai node la \texttt{[1,1]} va \texttt{[2,2]} dong gop $3 + 5 = 8$ tu nhanh trai. Quay lai root de di sang nhanh phai.}
...
\recolor{T.node["[3,5]"]}{state=dim}
\recolor{T.node["[5,5]"]}{state=dim}
\narrate{Ket qua: $\text{sum}(1,4) = 3 + 5 + 16 = 24$. Ba node la \texttt{[1,1]}, \texttt{[2,2]} va \texttt{[3,4]} (to \texttt{good}) thuc su dong gop vao ket qua. Cac node con lai hoac bi bo qua hoac chi la trung gian. Tong so node tham: $O(\log n)$.}
\end{animation}
