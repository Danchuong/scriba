...
\narrate{dp[1][2] = cost(1,2) = 2. Một trường cho nhà 1-2: trung vị có trọng số tại nhà 2 (trọng số 3 $\geq$ nửa của 5). Chi phí = 2*1 + 3*0 = 2.}
...
\narrate{dp[1][3] = cost(1,3) = 7. Trung vị có trọng số tại nhà 3. Chi phí = 2*2 + 3*1 + 5*0 = 7.}
...
\narrate{dp[1][4] = cost(1,4) = 8. Trung vị vẫn tại nhà 3 (trọng số tích lũy 10 $\geq$ 5.5). Chi phí = 2*2 + 3*1 + 5*0 + 1*1 = 8.}
...
\narrate{dp[1][5] = cost(1,5) = 16. Trung vị tại nhà 3. Chi phí = 2*2 + 3*1 + 5*0 + 1*1 + 4*2 = 16. Thêm nhà 5 ở xa làm chi phí tăng đáng kể.}
...
\narrate{dp[1][6] = cost(1,6) = 33. Trung vị dịch sang nhà 4. Chi phí = 2*3 + 3*2 + 5*1 + 1*0 + 4*1 + 6*2 = 33. Một trường cho tất cả các nhà rất tốn kém!}

\step
\recolor{dp.cell[1][6]}{state=done}
\narrate{Hàng j=1 hoàn thành. Với một trường, chi phí tăng nhanh khi đoạn mở rộng. Bây giờ ta tính hàng j=2: đặt hai trường. Công thức truy hồi là dp[2][i] = min theo m của dp[1][m] + cost(m+1, i).}
...
\narrate{dp[2][3]: thử m=1: dp[1][1] + cost(2,3) = 0 + 3 = 3 (trung vị tại 3, chi phí=3*1+5*0=3). Thử m=2: dp[1][2] + cost(3,3) = 2 + 0 = 2. Tốt nhất là m=2.}
...
\narrate{dp[2][4]: thử m=1 cho 4, m=2 cho dp[1][2]+cost(3,4) = 2+1 = 3 (trung vị tại 3, chi phí=5*0+1*1=1), m=3 cho 7. Tốt nhất tại m=2.}
...
\apply{cost_val.cell[0]}{value=1}
\narrate{dp[2][5]: m=3 cho dp[1][3]+cost(4,5) = 7+1 = 8 (trường tại 5, chi phí=1*1+4*0=1). m=4 cũng cho 8+0=8. Tốt nhất là 8.}
...
\narrate{dp[2][6]: m=4 cho dp[1][4]+cost(5,6) = 8+4 = 12 (trường tại 6, chi phí=4*1+6*0=4). Các cách chia khác: m=3 cho 13, m=5 cho 16. Tốt nhất tại m=4.}
