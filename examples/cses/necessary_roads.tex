...
\cursor{disc.cell}{1}
\cursor{low.cell}{1}
\narrate{Thăm đỉnh $2$. Cạnh cây $1 \to 2$. $disc[2] = 1$, $low[2] = 1$. Duyệt đỉnh kề $3$.}
...
\narrate{Thăm đỉnh $3$. Cạnh cây $2 \to 3$. $disc[3] = 2$, $low[3] = 2$.}
...
\narrate{Tiếp tục DFS từ $3$ sang $4$. Cạnh cây $3 \to 4$. $disc[4] = 3$, $low[4] = 3$. Đỉnh $4$ không có cạnh ngược nào đi lên.}
...
\narrate{DFS sang đỉnh $5$. Cạnh cây $4 \to 5$. $disc[5] = 4$, $low[5] = 4$.}
...
\narrate{DFS sang đỉnh $6$. Cạnh cây $5 \to 6$. $disc[6] = 5$, $low[6] = 5$.}
...
\narrate{DFS sang đỉnh $7$. Cạnh cây $6 \to 7$. $disc[7] = 6$, $low[7] = 6$.}
...
\narrate{Quay lui $7 \to 6$. $low[6] = \min(5, low[7]) = \min(5, 4) = 4$. Kiểm tra cầu: $low[7]=4 > disc[6]=5$? Không. Cạnh $6$-$7$ KHÔNG phải cầu.}

\step
\apply{low.cell[4]}{value="4"}
\recolor{low.cell[4]}{state=done}
\narrate{Quay lui $6 \to 5$. $low[5] = \min(4, low[6]) = \min(4, 4) = 4$. Kiểm tra cầu: $low[6]=4 > disc[5]=4$? Không (không lớn hơn nghiêm ngặt). Cạnh $5$-$6$ KHÔNG phải cầu.}

\step
\apply{low.cell[3]}{value="3"}
\recolor{low.cell[3]}{state=done}
\recolor{low.cell[4]}{state=error}
\recolor{disc.cell[3]}{state=error}
\recolor{g.edge[(4,5)]}{state=error}
\annotate{low.cell[4]}{label="4 > 3", arrow_from="disc.cell[3]", color=error}
\narrate{Quay lui $5 \to 4$. $low[4] = \min(3, low[5]) = \min(3, 4) = 3$. Kiểm tra cầu: $low[5]=4 > disc[4]=3$? ĐÚNG! Cạnh $(4,5)$ là CẦU. Không có cạnh ngược nào từ $\{5,6,7\}$ nối tới đỉnh $4$ hoặc cao hơn.}

\step
\recolor{g.edge[(3,4)]}{state=error}
\recolor{low.cell[3]}{state=error}
\recolor{disc.cell[2]}{state=error}
\annotate{low.cell[3]}{label="3 > 2", arrow_from="disc.cell[2]", color=error}
\narrate{Quay lui $4 \to 3$. $low[3]$ giữ nguyên $3$. Kiểm tra cầu: $low[4]=3 > disc[3]=2$? ĐÚNG! Cạnh $(3,4)$ cũng là CẦU. Đỉnh $4$ hoàn toàn không có cạnh ngược -- cây con của nó bị cô lập hoàn toàn khỏi các tổ tiên của $3$.}
...
\narrate{Quay lui $3 \to 2$. $low[2] = \min(1, low[3]) = \min(1, 0) = 0$. Kiểm tra cầu: $low[3]=0 > disc[2]=1$? Không. Cạnh $2$-$3$ an toàn. Quay lui $2 \to 1$. $low[1] = \min(0, 0) = 0$. Cạnh $1$-$2$: $low[2]=0 > disc[1]=0$? Không. DFS hoàn tất.}
