...
Run in CI (wired into pytest via tests/doc_coverage/test_ruleset_sync.py)
to catch drift between the ruleset and the implementation.
...
``### R-*`` card must carry at least one ``**Code ref:**`` and one
``**Test ref:**``.
...
RULESET = REPO_ROOT / "docs/spec/smart-label-ruleset.md"
...
def main() -> int:
    ...
    cards = list(re.finditer(r"^### (R-[0-9]+[a-z]?) — .+$", text, re.MULTILINE))
    if not cards:
        print("ERROR: no rule cards found")
        return 1
    ...
    print(
        f"OK: {len(cards)} rules, {len(code_refs)} code-refs"
