> **Scriba v0.26.4** — 3 renderers (TexRenderer, AnimationRenderer, DiagramRenderer), 21 primitive types, zero-JS output.
...
| [`environments.md`](spec/environments.md) | **Locked** spec for the `animation` and `diagram` environments: grammar, the 23 inner commands, the 21 primitives, the 9 semantic states, the Starlark host, the HTML/CSS contract, and the error catalog. Single source of truth for implementation. |
