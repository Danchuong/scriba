...
| E1107 | Value type mismatch on `\apply` — a non-numeric `value=` on a part whose value is intrinsically numeric (a Bar column height, a Matrix cell colour). There is no string-display mode to coerce into, so the value soft-drops server-side yet the differ bakes a `value_change` that stamps then reverts at runtime (a flip-back under `show_values`). | Pass an int or float, e.g. `\apply{h.bar[0]}{value=8}`; use a string container (Array) if you need to display arbitrary text. |
