...
`AnimationRenderer` uses the exact same primitive catalog as `DiagramRenderer` (see [`diagram-plugin.md`](diagram-plugin.md) §6 for the dispatch table). All 21 primitives — the six base primitives (`Array`, `Grid`, `DPTable`, `Graph`, `Tree`, `NumberLine`), the seven extended primitives (`Matrix`/`Heatmap`, `Stack`, `Plane2D`, `MetricPlot`, `Hypercube`, `Bar`, `Equation`), and the eight data-structure primitives (`CodePanel`, `HashMap`, `LinkedList`, `Queue`, `Deque`, `VariableWatch`, `Forest`, `TraceTable`) — emit the same `<g data-target="...">` groups with the same selector strings in both plugins. The only difference is that `AnimationRenderer` calls the emitter once per frame, producing N independent `<svg>` stages; `DiagramRenderer` calls it once.
