...
- **Editorial script**: `\begin{animation}` environment với `\step`, `\narrate`, `\apply`, `\recolor`, ...
- **`\compute{...}` block**: Starlark — chạy thuật toán + sinh event trace.
...
## Consuming the trace in the editorial script

The `\compute{...}` block produces a `trace` list where each entry has a `"type"` field (`"init"`, `"candidates"`, `"commit"`, `"traceback"`). The editorial `\step` blocks below the `\compute{...}` block consume that trace by referencing interpolated values via `${...}` expressions — one `\step` group per logical event type.

There is no `match` directive in Scriba's grammar. The pattern is simply to write one `\step` per event that needs visualization, using `${trace[i].field}` interpolation to pull values from the computed trace into `\apply`, `\annotate`, and `\narrate` arguments.

## Cái Scriba làm được tốt cho Frog 1

| Component | Scriba support |
|---|---|
| DP table fill với value per cell | ✅ `Array` + `\apply{cell}{value=...}` |
