...
## 9. Phụ lục — trả lời 3 câu hỏi của gap-dsu-forest (Forest Ph2 union-glide)

*Do team-lead khép vòng dependency, suy trực tiếp từ thiết kế §4-6 của case này.*

- **(i) Subtree-move = N `position_move` rời, không phải 1 khối.** Cơ chế
  `get_node_positions()` → differ per `data-target` (differ.py:218-236) phát mỗi node một
  tween riêng. Hệ quả thị giác: N node trượt đồng thời cùng duration + easing đọc như một
  khối cứng — đúng nhu cầu union. [Deduced] Cap thực dụng: dưới ~30 node/subtree (cùng cỡ
  trần force-layout hiện hành) trước khi rối; Forest DSU điển hình nhỏ hơn nhiều.
- **(ii) Identity sống qua reparent.** Identity key là `{shape}.node[{id}]` — node-id, không
  chứa parent (tiền lệ: Tree reparent tree.py:411 đã phát `position_move` cho node giữ
  nguyên key qua `_inject_tree_positions`, emitter.py:211). Reparent không phá identity.
  [Confirmed qua đường Tree hiện hành]
- **(iii) Đủ — không cần kind mới.** Sau khi sửa hình học `position_move` thành
  glide-to-new (§6), union-glide của Forest cưỡi nguyên kind 11, inverse có sẵn (A-2 đóng
  kín). Điều kiện duy nhất: Forest tuân A-6 (envelope prescan qua `_structural_prescan`
  như LinkedList) để bbox không nhảy giữa union. [Deduced]
