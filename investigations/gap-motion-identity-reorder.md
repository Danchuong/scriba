...
### 5.7 Dependencies — trả lời 3 câu hỏi motion của gap-dsu-forest (Forest primitive)

gap-dsu-forest hỏi 3 điều về union = reparent (`tree.py:411` `_reparent_internal` đã có).
Cả 3 đều **củng cố verdict**, không cần gì mới ngoài đúng một chỉnh hình học §5.5 — **với một
điều kiện ràng buộc identity** mà Forest phải tuân.

**(i) Subtree-move khi union/reparent = 1 motion khối hay N `position_move` rời?**
→ **N `position_move` rời, mỗi node một cái. [Confirmed]**
- Substrate là **per-target**: `get_node_positions()` (`tree.py:507-517`) trả một dict
  `{name.node[id]: (x,y)}`, **một entry mỗi node**; differ so `x`/`y` từng cell độc lập
  (`differ.py:218-236`) → phát **một `position_move` cho mỗi node đã dời**. Không có kind
  "group/block" nào trong registry (đóng ở 11, A-2) — muốn khối phải thêm kind, không nên.
- `_reparent_internal` gọi `_relayout()` (`tree.py:476` → `:275-285`) chạy
  **`_reingold_tilford` toàn cục**: sau reparent, MỌI node trong subtree được cấp toạ độ mới
  từ đầu (depth +Δ, x tính lại theo thứ tự sibling mới) → **không phải một offset cứng**. Một
  Transition "khối cứng" sẽ *sai* ở đây vì hình học nội bộ subtree đổi (giãn cách dưới parent
  mới). Vậy N glide độc lập là **đúng bản chất layout**, không phải giải pháp chắp vá.
- **Cảm giác khối cho không**: N glide dùng chung frame → chung `duration`/easing runtime, mắt
  vẫn đọc thành một subtree bay liền mạch dù mỗi node có delta riêng. Được "block-perception"
  mà không cần "block-kind". [Deduced]

**(ii) Element-identity có bảo toàn khi node đổi parent? Identity key có chứa parent?**
→ **Bảo toàn. Key KHÔNG chứa parent. [Confirmed] — nhưng Forest phải giữ đúng bất biến này.**
- `_node_key(node_id)` = `f"node[{node_id}]"` (`tree.py:497-499`) — **thuần node id**, không
  parent, không path. `data-target = {name}.node[{id}]`. `_reparent_internal` (`tree.py:411-476`)
  chỉ sửa `children_map`/`edges` rồi relayout; `node_id` bất biến suốt thao tác → qua frame
  reparent node **giữ nguyên `data-target`** → differ thấy **cùng key, `(x,y)` mới** →
  `position_move` **glide** (không phải `element_remove`+`element_add` = pop/fade).
- Nếu identity dựa trên **path/parent** (vd `left.right` hay `set[rootA].node[id]`) thì reparent
  **đổi key → phá identity → pop-out/pop-in, không có glide**. Tree tránh được vì key theo id
  nội tại. **Ràng buộc cho Forest (điều kiện của câu trả lời "bảo toàn"):** node identity =
  **id nội tại**, KHÔNG bao giờ nhét `parent`/`root`/DSU-set vào key. Khi union đổi root của một
  node, `data-target` của nó **phải vẫn** là `forest.node[id]` (scope theo tên forest ổn định,
  không theo root). Key theo root ⇒ mọi union phá identity toàn subtree. Đây là **cọc thiết kế
  duy nhất** Forest cần đóng. [Confirmed cơ chế / Hypothesized cho tên key Forest]

**(iii) `position_move` + fs-flag đã đủ cho union-glide, hay cần kind mới?**
→ **Đủ. Không cần kind mới. Chung đúng một fix hình học §5.5. [Confirmed đường dữ liệu /
Deduced hình học]**
- Đường dữ liệu đã trọn: `get_node_positions` → `_inject_tree_positions`
  (`emitter.py:202-228`, `hasattr` `:211`) → `position_move` (`differ.py:218-236`) → runtime.
  Tree đã ship nguyên đường này; **Forest là primitive họ-Tree, cưỡi đúng substrate** — zero
  đổi differ/emitter. `position_move` **là** kind đúng (self-inverse, registry giữ 11); fs-flag
  (`fs=1`) là lưới an toàn snap-to-truth.
- **Nhưng** hình học hiện tại kết-thúc-ở-ghế-cũ (`scriba.js:213-216`) + dựa fs-snap = "visible
  lurch" (`differ.py:280`; `motion-ruleset.md:119-120`). Reparent nhỏ thì lurch chấp nhận được
  (đúng hành vi Tree hôm nay); muốn **union-GLIDE sạch** thì áp fix §5.5 (glide-to-new, công
  thức `cursor_move` `scriba.js:318-321`). Forest **thừa hưởng chung đúng một fix** với A4/A5 —
  không thêm việc, không kind mới, **không thêm version bump** (vẫn là bump 17→18 chung).
- Envelope: prescan A-6 mà gap-dsu-forest nêu là **đúng lớp** để chống bbox nhảy khi forest
  đổi hình dạng — độc lập với motion kind (bài học `_envelope_n` của LinkedList, `linkedlist.py`).

**Tóm tắt phụ thuộc:** Forest phase 2 **không cần gì mới từ lớp motion** ngoài fix §5.5 (đã
tính trong bump 17→18), **với điều kiện** node identity keyed thuần id nội tại (câu ii). Verdict
Forest có thể chốt: reuse `position_move`, N-glide per node, không kind khối.
