...
- **Chỉ cầu persistent-mà-DI-CHUYỂN mới cần `link_move`.** Annotation **không diff position** (differ chỉ
  add/remove/recolor, `differ.py:319-374`), nên một link persistent đổi đầu mút mà muốn path **tái-neo liên tục**
  (không fade-out/in) vẫn cần một kind `link_move` (một handler + inverse + một bump) — đây là gap thật, **không**
  được A4 cấp tự động. **Tin tốt từ A4:** verdict A4 đã sửa `position_move` ends-at-old → **glide-to-new**, nên khi
  cần dựng `link_move` thì **copy đúng hình học glide-to-new của A4** (không phải phát minh lại; đối lập với thời
  `cursor_move` phải thêm vì `position_move` kết thúc ở ghế **cũ**, R-38 `smart-label-ruleset.md:578`). **Khuyến
  nghị vẫn HOÃN** (YAGNI): chưa bài nào trong A2c/C3 đòi path di chuyển liên tục — A2c ephemeral đã đủ (add/remove),
  C3 mapping cố định. **[Hypothesized]**
...
|---|---|---|---|---|
| **P0** | **A4 (đã chốt)** — marker=identity: caret `cursor_move` + plane2d sweep `position_move` glide-to-new (0 verb mới) | có (runtime, 1 handler) | **1 bump** (gộp) | — (gap-motion-identity) |
...
- **A4 (BẮT BUỘC cho C1, TUỲ CHỌN cho cầu) — ĐÃ CHỐT:** `gap-motion-identity.md`. Verdict: element-identity
  thắng, **KHÔNG phá R-32** (R-32 = envelope primitive bất biến, không cấm phần tử trượt trong envelope). Sweep
  line **tái dùng `position_move`** (0 kind mới) qua `\apply{p.line[i]}{to_x=…}` (0 verb mới); fix = `position_move`
  ends-at-old → glide-to-new (1 handler). Sweep intra-viewport không đổi bbox. Cầu **tĩnh/ephemeral KHÔNG cần A4**;
  chỉ *cầu di chuyển* (P4, đã hoãn) mới cần motion generalization — và khi đó **copy glide-to-new của A4** (§7).
...
2. **Reflow R-32 (THẤP — hạ từ TB sau verdict A4).** Đường nối cấp-stage vẽ *trên* shape, không vào
   `bounding_box()` → không nới envelope → không đe doạ R-32. Verdict A4 xác nhận R-32 chỉ ràng buộc **envelope
   primitive bất biến**, **không cấm phần tử trượt trong envelope** — nên cầu vẽ ở tầng stage càng an toàn. Nếu
   endpoint neo vào cell **dời** giữa scene thì path phải theo; với tĩnh/ephemeral (re-author mỗi frame + `fs=1`)
   là an toàn. **[Confirmed via A4 verdict]**
